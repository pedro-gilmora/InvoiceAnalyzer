...
public partial class AgentService(
    ...
    IToastService toastService) : ISourcesContainer
{
    ...
    public ICollection<MessageContent> Files => MessageFiles.Values;
...
#pragma warning restore CS8618
    ...
    {
        if (e is { ShiftKey: true, Code: "Enter" })
        {
            await QueryAsync();
        }
    }
    ...
    internal async Task QueryAsync()
    {
        ...
        try
        {
            ...
            if (Files.Count > 0)
            {
                await foreach (var answerChunk in memory.AskStreamingAsync($@"User says: {_q}. ""NoResult"" property should be true if not found relevant info", cancellationToken: MessageCancelSource.Token))
                {
                    if (answerChunk.NoResult) break;

                    chunckCount++;

                    if (answerChunk.Result is not { Length: > 0 } chunk) continue;
            ...
            if(chunckCount == 0)
            {
                await foreach (var content in chatService.GetStreamingChatMessageContentsAsync(ChatHistory, promptExecutionSettings, kernel, MessageCancelSource.Token))
                {
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
    public ICollection<MessageContent> Files { get; } = [];
}
...
{
    ICollection<MessageContent> Files { get; }
